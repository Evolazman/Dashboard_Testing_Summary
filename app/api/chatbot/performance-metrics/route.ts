...
export async function GET(request: Request) {
  try {
    ...
    let query = DatabaseService.supabase.from("chatbot_performance_metrics").select("*")
    ...
    console.error("Error fetching chatbot performance metrics:", error)
    return NextResponse.json(
      {
        success: false,
        error: "Failed to fetch chatbot performance metrics",
      },
      { status: 500 },
    )
  }
